# 世田谷ホワイトニング — Webアプリ（Phase 2まで完了）

オンライン診療から始めるホームホワイトニングサービスのWebアプリです。
[バックエンド構築ロードマップ](https://claude.ai/code/artifact/73237527-d822-4441-be64-f907c9b3bde8) の Phase 0（土台づくり）・Phase 1（会員登録とステータス表示）・Phase 2（今すぐ診療サイン）が完了し、**インターネット上に公開済み**の状態のプロジェクトです。
...
まだできないこと（Phase 3以降で作ります）：オンライン診療の結果入力・購入資格の判定、決済。
...
app/
  ...
  admin/(dashboard)/     … 管理画面本体（受付状況の切り替え）。ログインしていないとここには入れない
components/
...
バックエンド構築ロードマップに沿って、Phase 3（診療から承認まで）に進みます。「Phase 3から始めよう」と伝えれば続きに着手できます。
