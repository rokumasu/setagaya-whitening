# 世田谷ホワイトニング — Webアプリ（Phase 3まで完了）

オンライン診療から始めるホームホワイトニングサービスのWebアプリです。
[バックエンド構築ロードマップ](https://claude.ai/code/artifact/73237527-d822-4441-be64-f907c9b3bde8) の Phase 0（土台づくり）・Phase 1（会員登録とステータス表示）・Phase 2（今すぐ診療サイン）・Phase 3（診療から承認まで）が完了し、**インターネット上に公開済み**の状態のプロジェクトです。
...
まだできないこと（Phase 4以降で作ります）：決済（Stripe）、購入・発送管理。
...
app/
  ...
  admin/(dashboard)/     … 管理画面本体（受付状況の切り替え、会員一覧・診療結果の入力）。
                            ログインしていないとここには入れない
components/
...
バックエンド構築ロードマップに沿って、Phase 4（購入とStripe決済）に進みます。「Phase 4から始めよう」と伝えれば続きに着手できます。
