...
- 決済完了・発送完了のタイミングで、お客様へ自動でお知らせメールが届く（詳しくは下記「Phase 5」参照。独自ドメインのDNS反映待ちのため、現在はまだ実際には届かない）

まだできないこと（Phase 5の残りで作ります）：特定商取引法ページ・プライバシーポリシーの掲載。
...
- ✅ **注文確認・発送連絡メールの自動送信（実装済み）** … Resend（メール配信サービス）を使い、決済完了時・発送完了時にお客様へ自動でメールを送るコードを実装済み。独自ドメイン（`setagaya-whitening.com`）を取得しDNS設定も完了したが、**反映待ち（最大72時間程度）のため、まだ実際には届かない**。反映後にResend側でドメイン認証の確認が必要
- 🔲 特定商取引法ページ・プライバシーポリシーの掲載 … 後回しにする方針（事業者情報の記載方法が未決定のため）
...
Phase 5の残りとして、独自ドメインのDNS反映を待ってからメール送信の動作確認を行い、特定商取引法ページ・プライバシーポリシーの掲載を進めます。
